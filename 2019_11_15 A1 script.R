...
A1 = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/Paper Data Copies/2019_11_15 A1.csv")
  ## Remove the extra column
A1 = A1[,-5]
  ## Replace NAs with the ROI# for each ROI
A1[,1] = rep(1:max(A1$Field., na.rm = T), each = max(A1$Object.))
  ## Reshape the data into something that is usable for plotting for green
Green = reshape(data = A1, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")
  ## Reshape the data into something that is usable for plotting for red
Red = reshape(data = A1, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
A1bkgd = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/Paper Data Copies/2019_11_15 A1bkgd.csv")
A1bkgd = A1bkgd[,-5]
  ## Replace NAs with the ROI# for each ROI
A1bkgd[,1] = rep(1:max(A1bkgd$Field., na.rm = T), each = max(A1bkgd$Object.))
  ## Reshape the data into something that is usable for plotting for green
Greenbkgd = reshape(data = A1bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")
  ## Reshape the data into something that is usable for plotting for red
Redbkgd = reshape(data = A1bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
  ## Rename the "Fields" column to "t" (time)
names(Greenbkgd)[1] = "t"
names(Redbkgd)[1] = "t"
...
exposure_frame = cbind(c(1, 15, 45, 75, 105, 125), c(15, 45, 75, 105, 125, 140))
...
response_frame = cbind(c(16,76,126), c(75,125,140))
  ## Rename the start and end columns
