...
A3early = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/Paper Data Copies/2019_11_11 A3early.csv")
  ## Remove the extra column
A3early = A3early[,-5]
  ## Replace NAs with the ROI# for each ROI
A3early[,1] = rep(1:max(A3early$Field., na.rm = T), each = max(A3early$Object.))
  ## Reshape the data into something that is usable for plotting for green
Green = reshape(data = A3early, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")
  ## Reshape the data into something that is usable for plotting for red
Red = reshape(data = A3early, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
A3earlybkgd = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/Paper Data Copies/2019_11_11 A3earlybkgd.csv")
A3earlybkgd = A3earlybkgd[,-5]
  ## Replace NAs with the ROI# for each ROI
A3earlybkgd[,1] = rep(1:max(A3earlybkgd$Field., na.rm = T), each = max(A3earlybkgd$Object.))
  ## Reshape the data into something that is usable for plotting for green
Greenbkgd = reshape(data = A3earlybkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")
  ## Reshape the data into something that is usable for plotting for red
Redbkgd = reshape(data = A3earlybkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
  ## Rename the "Fields" column to "t" (time)
names(Greenbkgd)[1] = "t"
names(Redbkgd)[1] = "t"
...
A3earlyGreen = Green - AvgGbkgd
A3earlyRed = Red - AvgRbkgd
...
agonist = c("Ringer's", "5mM Beta-alanine", "Ringer's", "2.5uM LY344862")
  ## Create a matrix containing the frame number at which the agonist was added and turned off
exposure_frame = cbind(c(1, 13, 43, 73), c(13, 43, 73, 75))
...
response_frame = cbind(c(13,73), c(73,75))
  ## Rename the start and end columns
...
Agonists_df = data.frame(c(agonist[seq(2,length(agonist),2)]), c(0.10, 0.10))
# Rename the columns
...
Analysis = function(Agonists_information, Exposure_Information, Fluorescence_Information){
  ...
  #dir.create("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_11_11\\A3earlyplots")
  #for (i in seq_along(FLUO[,1:ncol(FLUO)])) {
    #tempname = paste("cell_",i,".png", sep="")
  #  outpath = file.path("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_11_11\\A3earlyplots", paste("cell_",i,".png", sep=""))
  #  png(filename = outpath)
  #  plot(t, FLUO[,i], xlim = c(t[1],t[length(t)]), ylim = c(0,2))
  #  dev.off()
  #}
