...
A3 = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/Paper Data Copies/2019_12_13 A3.csv")
  ## Remove the extra column
A3 = A3[,-5]
  ## Replace NAs with the ROI# for each ROI
A3[,1] = rep(1:max(A3$Field., na.rm = T), each = max(A3$Object.))

  ## Reshape the data into something that is usable for plotting for green
Green = reshape(data = A3, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")
  ## Reshape the data into something that is usable for plotting for red
Red = reshape(data = A3, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
A3bkgd = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/Paper Data Copies/2019_12_13 A3bkgd.csv")
A3bkgd = A3bkgd[,-5]
  ## Replace NAs with the ROI# for each ROI
A3bkgd[,1] = rep(1:max(A3bkgd$Field., na.rm = T), each = max(A3bkgd$Object.))

  ## Reshape the data into something that is usable for plotting for green
Greenbkgd = reshape(data = A3bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")
  ## Reshape the data into something that is usable for plotting for red
Redbkgd = reshape(data = A3bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
  ## Rename the "Fields" column to "t" (time)
names(Greenbkgd)[1] = "t"
names(Redbkgd)[1] = "t"
...
Greenbkgd = Greenbkgd[-186,-1]
Redbkgd = Redbkgd[-186,-1]
...
exposure_frame = cbind(c(1, 15, 55, 75, 125, 175), c(15, 55, 75, 125, 175, 194))
...
response_frame = cbind(c(16,76,176), c(75,175,194))
  ## Rename the start and end columns
...
Analysis = function(Agonists_information, Exposure_Information, Fluorescence_Information){
  ...
  for (i in 1:ncol(Agonist_threshold_df)){
    Agonist_responses[nrow(Agonists_df),i] = if_else(Agonist_responses[nrow(Agonists_df),i] == "Responder", "Neuron", 
                                                     if_else(FLUO[173,i] - baseline_df[nrow(Agonists_df),i] > Agonist_threshold_df[nrow(Agonists_df),i], "Neuron", "Non-neuron"))
  }
  ...
  dir.create("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_12_13\\A3plots")
  for (i in seq_along(FLUO[,1:ncol(FLUO)])) {
    #tempname = paste("cell_",i,".png", sep="")
    outpath = file.path("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_12_13\\A3plots", paste("cell_",i,".png", sep=""))
...
#write.csv(FLUO, "M:\\Erik\\Data\\Calcium Imaging Analysis\\For ANN\\12_13_A3_FLUO.csv", sep = ",", row.names = F, col.names = F)

FLUO = FLUO[, c(1,2,7,9,25,38,61)]

# Make plots of perfusion data
CaImaging = ggplot(data = FLUO, aes(x = FLUO$t)) +
  geom_line(data = FLUO, aes(y = FLUO$MEAN_GREEN.1.1), color = "red") +
  geom_line(data = FLUO, aes(y = FLUO$MEAN_GREEN.1.6), color = "forest green") +
  geom_line(data = FLUO, aes(y = FLUO$MEAN_GREEN.1.8), color = "purple") +
  geom_line(data = FLUO, aes(y = FLUO$MEAN_GREEN.1.24), color = "navy") +
  geom_line(data = FLUO, aes(y = FLUO$MEAN_GREEN.1.37), color = "gray") +
  geom_line(data = FLUO, aes(y = FLUO$MEAN_GREEN.1.60), color = "orange") +
  #geom_vline(xintercept = 30, color = "forest green") + 
  #geom_vline(xintercept = 120, color = "forest green") + 
  #geom_vline(xintercept = 210, color = "red") + 
  #geom_vline(xintercept = 300, color = "red") + 
  #geom_vline(xintercept = 390, color = "purple") + 
  #geom_line(color = "blue", size = 1) + 
  coord_cartesian(xlim = c(0,600), ylim = c(1,3.5)) + 
  theme_light() +
  labs(x = "Time (s)", y = "340:380 Normalized Fluorescence (a.u.)", title = "MUT DRG Responses to 5mM B-alanine, 1uM Capsaicin, 140mM K") +
