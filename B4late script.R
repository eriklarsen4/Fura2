...
B4 = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_10_30/B4late.csv")
...
exposure_frame = cbind(c(1, 8, 28, 48, 68), c(8, 28, 48, 68, 88))
...
response_frame = cbind(c(1,28,68), c(28,68,88))
# Rename the start and end columns
...
Agonists_df = data.frame(c(agonist[seq(1,length(agonist),2)]), c(0.50, 0.50, 1.5))
# Rename the columns
...
Analysis = function(Agonists_information, Exposure_Information, Fluorescence_Information){
  ...
  # Keep the same cell #
  rownames(Agonist_threshold_df) = c(agonist[seq(1,length(agonist),2)])
  ...
  for (i in 1:ncol(Agonist_threshold_df)){
    Agonist_responses[nrow(Agonists_df),i] = if_else(Agonist_responses[nrow(Agonists_df),i] == "Responder", "Neuron", 
                                                     if_else(FLUO[78,i] - baseline_df[nrow(Agonists_df),i] > Agonist_threshold_df[nrow(Agonists_df),i], "Neuron", "Non-neuron"))
  }
  ...
  dir.create("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_10_30\\B4lateplots")
  for (i in seq_along(FLUO[,1:ncol(FLUO)])) {
    #tempname = paste("cell_",i,".png", sep="")
    outpath = file.path("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_10_30\\B4lateplots", paste("cell_",i,".png", sep=""))
