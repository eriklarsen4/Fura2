...
C1early = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_12_13/C1early.csv")


# Remove the extra column that gives the wrong (?) ratio
C1early = C1early[,-5]
# Replace NAs with the cell number for each cell
C1early[,1] = rep(1:max(C1early$Field., na.rm = T), each = max(C1early$Object.))

# Reshape the data into something that is usable for plotting for green
Green = reshape(data = C1early, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

# Reshape the data into something that is usable for plotting for red
Red = reshape(data = C1early, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
C1earlybkgd = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_12_13/C1earlybkgd.csv")

C1earlybkgd = C1earlybkgd[,-5]

# Replace NAs with the cell number for each cell
C1earlybkgd[,1] = rep(1:max(C1earlybkgd$Field., na.rm = T), each = max(C1earlybkgd$Object.))

# Reshape the data into something that is usable for plotting for green
Greenbkgd = reshape(data = C1earlybkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

# Reshape the data into something that is usable for plotting for red
Redbkgd = reshape(data = C1earlybkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
C1earlyGreen = Green - AvgGbkgd
C1earlyRed = Red - AvgRbkgd
...
agonist = c("Ringer's", "200uM AITC", "Ringer's", "1uM Capsaicin")

# Create a matrix containing the frame number at which the agonist was added and turned off
exposure_frame = cbind(c(1, 8, 38, 68), c(8, 38, 68, 77))
...
response_frame = cbind(c(8,68), c(68,77))
# Rename the start and end columns
...
Agonists_df = data.frame(c(agonist[seq(2,length(agonist),2)]), c(0.50, 0.50))
# Rename the columns
...
Analysis = function(Agonists_information, Exposure_Information, Fluorescence_Information){
  ...
  baseline_df[nrow(baseline_df),] = colSums(FLUO[exposure_frame[length(agonist)-2,1]:exposure_frame[length(agonist)-2,2], 1:length(FLUO)], dims = 1)/(exposure_frame[length(agonist)-2,2]-exposure_frame[length(agonist)-2,1])
  # Fill with the Hi K
  ...
  #for (i in 1:ncol(Agonist_threshold_df)){
   # Agonist_responses[nrow(Agonists_df),i] = if_else(Agonist_responses[nrow(Agonists_df),i] == "Responder", "Neuron", 
    #                                                 if_else(FLUO[137,i] - baseline_df[nrow(Agonists_df),i] > Agonist_threshold_df[nrow(Agonists_df),i], "Neuron", "Non-neuron"))
  #}
  ...
  Response_Stats_df = matrix(nrow = length(agonist)/2, ncol = 3)
  # Rename columns and rows
  rownames(Response_Stats_df) = c(agonist[seq(2,length(agonist),2)])
  colnames(Response_Stats_df) = c("# of Responders", "% of ROIs responding", "Maximum Response Magnitude")
  # Remove NAs
  ...
  for (i in 1:(nrow(Agonist_threshold_df))){
    ...
    Response_Stats_df[i,3] = max(FLUO[response_frame[i,1]:response_frame[i,2],])
  }
  ...
  for (i in 1:(nrow(Agonist_threshold_df)-1)){
    ...
    Response_Stats_df[i,2] = (length(which(Agonist_responses[i,] == "Responder"))/length(ncol(FLUO)))*100
  }
 ...
 # for (i in nrow(Agonists_df)){
  ...
  dir.create("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_12_13\\C1earlyplots")
  for (i in seq_along(FLUO[,1:ncol(FLUO)])) {
    #tempname = paste("cell_",i,".png", sep="")
    outpath = file.path("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_12_13\\C1earlyplots", paste("cell_",i,".png", sep=""))
...
# Export the dataframe for downstream analysis in Python
write.csv(FLUO, "M:\\Erik\\Data\\Calcium Imaging Analysis\\For ANN\\12_13_C1early_FLUO.csv", sep = ",", row.names = F, col.names = F)

# Make plots of perfusion data
CaImaging = ggplot(data = FLUO, aes(x = FLUO$t, y = FLUO$MEAN_GREEN.1.45)) + geom_vline(xintercept = 30, color = "forest green") + geom_vline(xintercept = 120, color = "forest green") + geom_vline(xintercept = 210, color = "red") + geom_vline(xintercept = 300, color = "red") + geom_vline(xintercept = 390, color = "purple") + geom_line(color = "blue") + coord_cartesian(xlim = c(0,450), ylim = c(0,3)) + labs(x = "Time (s)", y = "340:380 Normalized Fluorescence (a.u.)", title = "MUT DRG Responses to 2.5uM LY344862, 200uM AITC, 140mM K") + theme(plot.title = element_text(hjust = 0.5)) + theme_light() + geom_hline(yintercept = 1.0, color = "black", linetype = "solid")
