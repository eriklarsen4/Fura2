...
A4 = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_10_30/A4.csv")


# Remove the extra column that gives the wrong (?) ratio
A4 = A4[,-5]
# Replace NAs with the cell number for each cell
A4[,1] = rep(1:max(A4$Field., na.rm = T), each = max(A4$Object.))

# Reshape the data into something that is usable for plotting for green
Green = reshape(data = A4, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

# Reshape the data into something that is usable for plotting for red
Red = reshape(data = A4, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
A4bkgd = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_10_30/A4bkgd.csv")

A4bkgd = A4bkgd[,-5]

# Replace NAs with the cell number for each cell
A4bkgd[,1] = rep(1:max(A4bkgd$Field., na.rm = T), each = max(A4bkgd$Object.))

# Reshape the data into something that is usable for plotting for green
Greenbkgd = reshape(data = A4bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

# Reshape the data into something that is usable for plotting for red
Redbkgd = reshape(data = A4bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
exposure_frame = cbind(c(1, 8, 28, 59, 88, 118, 148, 168), c(8, 28, 59, 88, 118, 148, 168, 193))
...
response_frame = cbind(c(8,59,118,168), c(59,118,168,193))
# Rename the start and end columns
...
Analysis = function(Agonists_information, Exposure_Information, Fluorescence_Information){
  ...
  for (i in 1:ncol(Agonist_threshold_df)){
    Agonist_responses[nrow(Agonists_df),i] = if_else(Agonist_responses[nrow(Agonists_df),i] == "Responder", "Neuron", 
                                                     if_else(FLUO[179,i] - baseline_df[nrow(Agonists_df),i] > Agonist_threshold_df[nrow(Agonists_df),i], "Neuron", "Non-neuron"))
  }
  ...
  for (i in seq_along(FLUO[,1:ncol(FLUO)])) {
    ...
    outpath = file.path("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_10_30\\A4plots", paste("cell_",i,".png", sep=""))
