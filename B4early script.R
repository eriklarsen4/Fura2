...
B4 = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_10_30/B4early.csv")
...
B4bkgd = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_10_30/B4bkgd.csv")
...
agonist = c("Ringer's", "5mM B-Ala", "Ringer's", "2.5uM LY344862", "Ringer's", "25uM CYM5442", "Ringer's", "200uM AITC")

# Create a matrix containing the frame number at which the agonist was added and turned off
exposure_frame = cbind(c(1, 8, 28, 48, 68, 88, 108, 128), c(8, 28, 48, 68, 88, 108, 128, 137))
...
response_frame = cbind(c(8,48,88,128), c(48,88,128,137))
# Rename the start and end columns
...
Agonists_df = data.frame(c(agonist[seq(2,length(agonist),2)]), c(0.50, 0.50, 0.5, 0.5))
# Rename the columns
...
Analysis = function(Agonists_information, Exposure_Information, Fluorescence_Information){
  ...
  #                                                   if_else(FLUO[138,i] - baseline_df[nrow(Agonists_df),i] > Agonist_threshold_df[nrow(Agonists_df),i], "Neuron", #"Non-neuron"))
  #}
  ...
  colnames(Response_Stats_df) = c("# of Responders", "% Responders of ROIs", "Maximum Response Magnitude")
  # Remove NAs
  ...
  for (i in 1:(nrow(Agonist_threshold_df)-1)){
   ...
   Response_Stats_df[i,2] = (length(which(Agonist_responses[i,] == "Responder"))/ncol(FLUO))*100
  }
  
  
  for (i in 1:ncol(Response_Stats_df)){
    for (j in 1:nrow(Response_Stats_df)){
      # Find the percent of responders in the field
      Response_Stats_df[nrow(Response_Stats_df),2] =  (Response_Stats_df[nrow(Response_Stats_df),1]/ncol(FLUO))*100
    }
  }
  ...
  dir.create("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_10_30\\B4earlyplots")
  for (i in seq_along(FLUO[,1:ncol(FLUO)])) {
    #tempname = paste("cell_",i,".png", sep="")
    outpath = file.path("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_10_30\\B4earlyplots", paste("cell_",i,".png", sep=""))
