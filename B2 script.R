...
B2 = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_10_30/B2.csv")


# Remove the extra column that gives the wrong (?) ratio
B2 = B2[,-5]
# Replace NAs with the cell number for each cell
B2[,1] = rep(1:max(B2$Field., na.rm = T), each = max(B2$Object.))

# Reshape the data into something that is usable for plotting for green
Green = reshape(data = B2, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

# Reshape the data into something that is usable for plotting for red
Red = reshape(data = B2, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
B2bkgd = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_10_30/B2bkgd.csv")

B2bkgd = B2bkgd[,-5]

# Replace NAs with the cell number for each cell
B2bkgd[,1] = rep(1:max(B2bkgd$Field., na.rm = T), each = max(B2bkgd$Object.))

# Reshape the data into something that is usable for plotting for green
Greenbkgd = reshape(data = B2bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

# Reshape the data into something that is usable for plotting for red
Redbkgd = reshape(data = B2bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
agonist = c("Ringer's", "2.5uM LY344862", "Ringer's", "25uM CYM5442", "Ringer's", "1uM Cap", "Ringer's", "HiK")

# Create a matrix containing the frame number at which the agonist was added and turned off
exposure_frame = cbind(c(1, 8, 28, 53, 73, 99, 124, 148), c(8, 28, 53, 73, 99, 124, 148, 168))
...
response_frame = cbind(c(8,53,99,148), c(53,99,148,168))
# Rename the start and end columns
...
Agonists_df = data.frame(c(agonist[seq(2,length(agonist),2)]), c(0.50, 0.50, 0.5, 1.5))
# Rename the columns
...
Analysis = function(Agonists_information, Exposure_Information, Fluorescence_Information){
  ...
  # This has been revised to include "wash out", where Ringer's solution is added inbetween agonists, and is therefore included in agonist information
  ...
  # Make response calls to High Potassium addition
  for (i in 1:ncol(Agonist_threshold_df)){
    Agonist_responses[nrow(Agonists_df),i] = if_else(Agonist_responses[nrow(Agonists_df),i] == "Responder", "Neuron", 
                                                     if_else(FLUO[159,i] - baseline_df[nrow(Agonists_df),i] > Agonist_threshold_df[nrow(Agonists_df),i], "Neuron", "Non-neuron"))
  }
  ...
  colnames(Response_Stats_df) = c("# of Responders", "% of Neurons responding", "% Neurons of ROIs", "Maximum Response Magnitude", "Response Gain")
  # Remove NAs
  ...
  dir.create("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_10_30\\B2plots")
  for (i in seq_along(FLUO[,1:ncol(FLUO)])) {
    #tempname = paste("cell_",i,".png", sep="")
    outpath = file.path("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_10_30\\B2plots", paste("cell_",i,".png", sep=""))
...
CaImaging = ggplot(data = FLUO, aes(x = FLUO$t, y = FLUO$MEAN_GREEN.1.35)) + geom_vline(xintercept = 30, color = "forest green") + geom_vline(xintercept = 90, color = "forest green") + geom_vline(xintercept = 165, color = "orange") + geom_vline(xintercept = 225, color = "orange") + geom_vline(xintercept = 303, color = "red") + geom_vline(xintercept =378, color = "red") + geom_vline(xintercept = 450, color = "purple") + geom_line(color = "blue") + coord_cartesian(xlim = c(0,510), ylim = c(0,3)) + labs(x = "Time (s)", y = "340:380 Normalized Fluorescence (a.u.)", title = "WT DRG Responses to 2.5uM LY, 25uM CYM, 1uM Cap, 140mM K") + theme(plot.title = element_text(hjust = 0.5)) + theme_light() + geom_hline(yintercept = 1.0, color = "black", linetype = "solid")
