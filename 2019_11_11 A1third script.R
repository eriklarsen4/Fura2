...
A1third = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_11_11/A1third.csv")


# Remove the extra column that gives the wrong (?) ratio
A1third = A1third[,-5]
# Replace NAs with the cell number for each cell
A1third[,1] = rep(1:max(A1third$Field., na.rm = T), each = max(A1third$Object.))

# Reshape the data into something that is usable for plotting for green
Green = reshape(data = A1third, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

# Reshape the data into something that is usable for plotting for red
Red = reshape(data = A1third, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
t = ((1:nrow(Green))*3 + 381)


##### Background Fluorescence Values Acquisition and Prep #####

# Repeat above steps for the background ROIs
A1thirdbkgd = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_11_11/A1thirdbkgd.csv")

A1thirdbkgd = A1thirdbkgd[,-5]

# Replace NAs with the cell number for each cell
A1thirdbkgd[,1] = rep(1:max(A1thirdbkgd$Field., na.rm = T), each = max(A1thirdbkgd$Object.))

# Reshape the data into something that is usable for plotting for green
Greenbkgd = reshape(data = A1thirdbkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

# Reshape the data into something that is usable for plotting for red
Redbkgd = reshape(data = A1thirdbkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
Greenbkgd = Greenbkgd[-c(1:128),-1]
Redbkgd = Redbkgd[-c(1:128),-1]
...
A1thirdGreen = Green - AvgGbkgd
A1thirdRed = Red - AvgRbkgd
...
response_frame = cbind(c(23), c(43))
# Rename the start and end columns
...
Agonists_df = data.frame(c(agonist[seq(2,length(agonist),2)]), c(1.5))
# Rename the columns
...
Analysis = function(Agonists_information, Exposure_Information, Fluorescence_Information){
  ...
  for (i in 1:ncol(Agonist_threshold_df)){
    Agonist_responses[nrow(Agonists_df),i] = if_else(Agonist_responses[nrow(Agonists_df),i] == "Responder", "Neuron", 
                                                     if_else(FLUO[34,i] - baseline_df[nrow(Agonists_df),i] > Agonist_threshold_df[nrow(Agonists_df),i], "Neuron", "Non-neuron"))
  }
 ...
 # for (i in 1:(nrow(Agonist_threshold_df))){
  ...
  dir.create("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_11_11\\A1thirdplots")
  for (i in seq_along(FLUO[,1:ncol(FLUO)])) {
    #tempname = paste("cell_",i,".png", sep="")
    outpath = file.path("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_11_11\\A1thirdplots", paste("cell_",i,".png", sep=""))
