...
B3_2 = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/Paper Data Copies/2019_12_4 B3_2.csv")
  ## Remove the extra column
B3_2 = B3_2[,-5]
  ## Replace NAs with the ROI# for each ROI
B3_2[,1] = rep(1:max(B3_2$Field., na.rm = T), each = max(B3_2$Object.))
  ## Reshape the data into something that is usable for plotting for green
Green = reshape(data = B3_2, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")
  ## Reshape the data into something that is usable for plotting for red
Red = reshape(data = B3_2, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
B3_2bkgd = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/Paper Data Copies/2019_12_4 B3_2bkgd.csv")
B3_2bkgd = B3_2bkgd[,-5]
  ## Replace NAs with the ROI# for each ROI
B3_2bkgd[,1] = rep(1:max(B3_2bkgd$Field., na.rm = T), each = max(B3_2bkgd$Object.))
  ## Reshape the data into something that is usable for plotting for green
Greenbkgd = reshape(data = B3_2bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")
  ## Reshape the data into something that is usable for plotting for red
Redbkgd = reshape(data = B3_2bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
  ## Rename the "Fields" column to "t" (time)
names(Greenbkgd)[1] = "t"
names(Redbkgd)[1] = "t"
...
B3_2Green = Green - AvgGbkgd
B3_2Red = Red - AvgRbkgd
...
agonist = c("Ringer's", "2.5uM LY344864", "Ringer's", "1uM Capsaicin", "Ringer's", "HiK")
  ## Create a matrix containing the frame number at which the agonist was added and turned off
exposure_frame = cbind(c(1, 10, 40, 70, 100, 130), c(10, 40, 70, 100, 130, 150))
...
response_frame = cbind(c(11,71,131), c(70,130,150))
  ## Rename the start and end columns
