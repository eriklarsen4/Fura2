...
C1 = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_10_30/C1.csv")


# Remove the extra column that gives the wrong (?) ratio
C1 = C1[,-5]
# Replace NAs with the cell number for each cell
C1[,1] = rep(1:max(C1$Field., na.rm = T), each = max(C1$Object.))

# Reshape the data into something that is usable for plotting for green
Green = reshape(data = C1, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

# Reshape the data into something that is usable for plotting for red
Red = reshape(data = C1, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
C1bkgd = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_10_30/C1bkgd.csv")

C1bkgd = C1bkgd[,-5]

# Replace NAs with the cell number for each cell
C1bkgd[,1] = rep(1:max(C1bkgd$Field., na.rm = T), each = max(C1bkgd$Object.))

# Reshape the data into something that is usable for plotting for green
Greenbkgd = reshape(data = C1bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

# Reshape the data into something that is usable for plotting for red
Redbkgd = reshape(data = C1bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
agonist = c("Ringer's", "2.5uM LY344862", "Ringer's", "25uM CYM5442", "Ringer's", "200uM AITC", "Ringer's", "HiK")

# Create a matrix containing the frame number at which the agonist was added and turned off
exposure_frame = cbind(c(1, 8, 28, 48, 68, 88, 108, 128), c(8, 28, 48, 68, 88, 108, 128, 148))
...
response_frame = cbind(c(8,48,88,128), c(48,88,128,148))
# Rename the start and end columns
...
Agonists_df = data.frame(c(agonist[seq(2,length(agonist),2)]), c(0.50, 0.50, 0.50, 1.5))
# Rename the columns
...
Analysis = function(Agonists_information, Exposure_Information, Fluorescence_Information){
  ...
  for (i in 1:ncol(Agonist_threshold_df)){
    Agonist_responses[nrow(Agonists_df),i] = if_else(Agonist_responses[nrow(Agonists_df),i] == "Responder", "Neuron", 
                                                     if_else(FLUO[144,i] - baseline_df[nrow(Agonists_df),i] > Agonist_threshold_df[nrow(Agonists_df),i], "Neuron", "Non-neuron"))
  }
  ...
  dir.create("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_10_30\\C1plots")
  for (i in seq_along(FLUO[,1:ncol(FLUO)])) {
    #tempname = paste("cell_",i,".png", sep="")
    outpath = file.path("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_10_30\\C1plots", paste("cell_",i,".png", sep=""))
...
CaImaging = ggplot(data = FLUO, aes(x = FLUO$t, y = FLUO$MEAN_GREEN.1.24)) + geom_vline(xintercept = 30, color = "forest green") + geom_vline(xintercept = 120, color = "forest green") + geom_vline(xintercept = 180, color = "red") + geom_vline(xintercept = 240, color = "red") + geom_vline(xintercept = 315, color = "purple") + geom_point(alpha = 0.4, color = "blue") + coord_cartesian(xlim = c(0,444), ylim = c(0,3)) + labs(x = "Time (s)", y = "340:380 Normalized Fluorescence (a.u.)", title = "In vitro WT DRG Calcium Imaging Responses to 1mM B-Ala, 5mM B-Ala, 140mM K") + theme(plot.title = element_text(hjust = 0.5)) + theme_light() + geom_hline(yintercept = 1.0, color = "black", linetype = "solid")
