...
B1 = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_10_30/B1.csv")


# Remove the extra column that gives the wrong (?) ratio
B1 = B1[,-5]
# Replace NAs with the cell number for each cell
B1[,1] = rep(1:max(B1$Field., na.rm = T), each = max(B1$Object.))

# Reshape the data into something that is usable for plotting for green
Green = reshape(data = B1, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

# Reshape the data into something that is usable for plotting for red
Red = reshape(data = B1, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
B1bkgd = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/2019_10_30/B1bkgd.csv")

B1bkgd = B1bkgd[,-5]

# Replace NAs with the cell number for each cell
B1bkgd[,1] = rep(1:max(B1bkgd$Field., na.rm = T), each = max(B1bkgd$Object.))

# Reshape the data into something that is usable for plotting for green
Greenbkgd = reshape(data = B1bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

# Reshape the data into something that is usable for plotting for red
Redbkgd = reshape(data = B1bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
agonist = c("Ringer's", "2.5uM LY344862", "Ringer's", "5mM B-Ala", "Ringer's", "1uM Cap", "Ringer's", "HiK")

# Create a matrix containing the frame number at which the agonist was added and turned off
exposure_frame = cbind(c(1, 8, 28, 58, 78, 108, 128, 158), c(8, 28, 58, 78, 108, 128, 158, 178))
...
response_frame = cbind(c(8,58,108,158), c(58,108,158,178))
# Rename the start and end columns
...
Analysis = function(Agonists_information, Exposure_Information, Fluorescence_Information){
  ...
  for (i in 1:ncol(Agonist_threshold_df)){
    Agonist_responses[nrow(Agonists_df),i] = if_else(Agonist_responses[nrow(Agonists_df),i] == "Responder", "Neuron", 
                                                     if_else(FLUO[174,i] - baseline_df[nrow(Agonists_df),i] > Agonist_threshold_df[nrow(Agonists_df),i], "Neuron", "Non-neuron"))
  }
  ...
  dir.create("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_10_30\\B1plots")
  for (i in seq_along(FLUO[,1:ncol(FLUO)])) {
    #tempname = paste("cell_",i,".png", sep="")
    outpath = file.path("M:\\Erik\\Data\\Calcium Imaging Analysis\\Fura2 Images\\2019_10_30\\B1plots", paste("cell_",i,".png", sep=""))
