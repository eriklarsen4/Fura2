...
C2 = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/Paper Data Copies/2019_12_13 C2.csv")


  ## Remove the extra column that gives the wrong (?) ratio
C2 = C2[,-5]
  ## Replace NAs with the ROI# for each ROI
C2[,1] = rep(1:max(C2$Field., na.rm = T), each = max(C2$Object.))

  ## Reshape the data into something that is usable for plotting for green
Green = reshape(data = C2, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")

  ## Reshape the data into something that is usable for plotting for red
Red = reshape(data = C2, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
...
C2bkgd = read.csv("M:/Erik/Data/Calcium Imaging Analysis/Fura2 Images/Paper Data Copies/2019_12_13 C2bkgd.csv")

C2bkgd = C2bkgd[,-5]

  ## Replace NAs with the ROI# for each ROI
C2bkgd[,1] = rep(1:max(C2bkgd$Field., na.rm = T), each = max(C2bkgd$Object.))
  ## Reshape the data into something that is usable for plotting for green
Greenbkgd = reshape(data = C2bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_RED.1")
  ## Reshape the data into something that is usable for plotting for red
Redbkgd = reshape(data = C2bkgd, idvar = "Field.", timevar = "Object.", direction = "wide", drop = "MEAN_GREEN.1")
  ## Rename the "Fields" column to "t" (time)
names(Greenbkgd)[1] = "t"
names(Redbkgd)[1] = "t"
...
exposure_frame = cbind(c(1, 10, 40, 70, 100, 130), c(10, 40, 70, 100, 130, 155))
...
response_frame = cbind(c(11,71,131), c(70,130,155))
  ## Rename the start and end columns
